---
title: "Публикации и книги"
permalink: /publications/
layout: single
author_profile: true
---


## Статьи (в процессе рецензирования)

### Основная статья

**Kalinin VS.** Reflexive index φ as a heuristic model of consciousness formalizability: validation on 265,956 epochs and clinical perspectives.

**Журнал:** eLife (планируется подача)
**Статус:** Планируется подача
**Препринт:** DOI [10.5281/zenodo.21322891](https://doi.org/10.5281/zenodo.21322891)
**Ключевые результаты:**
  - AUC = 0.947 для детекции LIS vs healthy
  - AUC = 0.887 при кросс-популяционной валидации
  - Подтверждение параметра A (Awareness): p = 0.045
  - Специфичность: контрольные задачи дают AUC ≈ 0.5

📄 [Полный текст (Article.md)](https://github.com/Kalera77/cognitive-shadow-theory/blob/main/ru/EMPIRICAL/Article.md)

---

## Препринты

### Теоретическое ядро

**Kalinin VS.** Cognitive Shadow: Formal Limits of Consciousness Digitization (core).

- **Дата:** 10 апреля 2026 г. (обновлено 4 июля 2026 г.)
- **Содержание:** Аксиомы A1–A8*, Теорема 1′, формальная верификация в Coq
- **Лицензия:** CC BY‑NC 4.0 (текст), MIT (код)

📄 [cognitive_shadow_core.md](https://github.com/Kalera77/cognitive-shadow-theory/blob/main/ru/THEORY/cognitive_shadow_core.md)

### Эмпирическая валидация

**Kalinin VS.** Reflexive index φ as a heuristic model of consciousness formalizability: validation on 265,956 epochs.

- **Опубликован на Zenodo:** 10.5281/zenodo.21322891
- **Содержание:** полная эмпирическая статья с результатами валидации
  
### Динамическое расширение

**Kalinin VS.** Cognitive Shadow Dynamics: Extensions and Interfaces.

- **Дата:** 8 июля 2026 г.
- **Содержание:** Аксиомы A9–A29, Теоремы 2–10, динамика интерфейсов
- **Новое:** Теорема 10 (принцип наблюдаемости сигнатур)

📄 [cognitive_shadow_dynamics.md](https://github.com/Kalera77/cognitive-shadow-theory/blob/main/ru/THEORY/cognitive_shadow_dynamics.md)

### Методологический уровень

**Kalinin VS.** Formal Theory of Observable Boundaries of Consciousness.

- **Дата:** 3 июля 2026 г.
- **Содержание:** Кантовский разворот, пределы реконструкции, связь с физикой

📄 [Formal_Theory_of_Observable_Boundaries.md](https://github.com/Kalera77/cognitive-shadow-theory/blob/main/ru/THEORY/Formal_Theory_of_Observable_Boundaries_of_Consciousness.md)

### Манифест зрелости

**Kalinin VS.** MATURE_STATUS.md: Эпистемологическая декларация зрелости научной программы.

- **Дата:** 7 июля 2026 г.
- **Содержание:** Честность об ограничениях, фальсифицируемость, приглашение к сотрудничеству

📄 [MATURE_STATUS.md](https://github.com/Kalera77/cognitive-shadow-theory/blob/main/ru/THEORY/MATURE_STATUS.md)

### Этика

**Kalinin VS.** Shadow Ethics: Formal Principles for Cognitive Systems.

- **Дата:** 8 июля 2026 г.
- **Содержание:** Шесть принципов Shadow Ethics, формальная верификация

📄 [Shadow_Ethics_Formal_Principles.md](https://github.com/Kalera77/cognitive-shadow-theory/blob/main/ru/ETHICS/Shadow_Ethics_Formal_Principles_for_Cognitive_Systems.md)

**Kalinin VS.** Формальная этика когнитивных систем: от принципов к протоколам.

- **Дата:** 4 июля 2026 г.
- **Содержание:** Инженерная спецификация, протоколы применения

📄 [Formal_Ethics_Specification.md](https://github.com/Kalera77/cognitive-shadow-theory/blob/main/ru/ETHICS/Formal_Ethics_Specification.md)

### Приложения

- **Appendix S1:** Coq Listings — формальная верификация
- **Appendix S2:** Experimental Protocols — протоколы экспериментов
- **Appendix S3:** Empirical Roadmap — эмпирическая дорожная карта
- **Appendix S5:** Empirical Foundations — эмпирические основания
- **Appendix S6:** Reflexive Ethics — рефлексивная этика

📄 [Все приложения](https://github.com/Kalera77/cognitive-shadow-theory)

---

## Философские эссе

### Непостижимость как дар

Философское эссе, обосновывающее принятие неполноты формализации как условия свободы.

- **Жанр:** Философия, когнитивистика
- **Ключевая идея:** Неполнота формализации — не дефект, а условие свободы

### Книга вопросов

Эссе о системном мышлении и искусстве задавать вопросы.

- **Жанр:** Системное мышление, самопознание
- **Ключевая идея:** Вопросы важнее ответов

---

## Дополнительные материалы

### Руководство по осознанному использованию ИИ

**AI_Hygiene_Guide_ru.md** — практическое пособие по развитию когнитивных способностей и избеганию их деградации при использовании ИИ.

📄 [AI_Hygiene_Guide_ru.md](https://github.com/Kalera77/cognitive-shadow-theory/blob/main/ru/ETHICS/AI_Hygiene_Guide_ru.md)

### Пояснения к ключевым вопросам

**Clarifications_on_Key_Questions.md** — честные ответы на 13 критических вопросов о теории.

📄 [Clarifications_on_Key_Questions.md](https://github.com/Kalera77/cognitive-shadow-theory/blob/main/ru/EMPIRICAL/Clarifications_on_Key_Questions.md)

### Экспериментальная программа

**Experimental_Program.md** — план верификации теории когнитивной тени.

📄 [Experimental_Program.md](https://github.com/Kalera77/cognitive-shadow-theory/blob/main/ru/EMPIRICAL/Experimental_Program.md)

---

## Формальная верификация

Все теоремы формально верифицированы в **Coq 8.18+** и **TLA⁺**:

- **27 модулей Coq**, все теоремы доказаны конструктивно
- **TLA⁺ спецификации**, проверены на 100% состояний
- **Извлечение кода:** OCaml → FFI в Rust/Python

📄 [Documentation of Coq Modules](https://github.com/Kalera77/cognitive-shadow-theory/blob/main/ru/docs/Documentation_of_Coq_Modules.md)  
📄 [Исходный код (src/)](https://github.com/Kalera77/cognitive-shadow-theory/tree/main/src)

---

## Репозиторий

📦 **[github.com/Kalera77/cognitive-shadow-theory](https://github.com/Kalera77/cognitive-shadow-theory)**

**Лицензия:**
- Тексты: CC BY‑NC 4.0
- Код: MIT
- Коммерческие материалы: проприетарная лицензия (по запросу)